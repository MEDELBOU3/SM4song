...
async function initializeSearch() {
    ...
    document.querySelectorAll('.filter-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            if (currentQuery) {
                searchSpotify(currentQuery, e.target.dataset.type);
            }
        });
    });
...
async function getSearchSuggestions(query) {
    ...
    try {
        const data = await fetchSpotifyData(`search?q=${encodeURIComponent(query)}&type=artist,track,album&limit=5`);
        
        let suggestionsHTML = '';
        
        // Add recent searches
        if (recentSearches.size > 0) {
            suggestionsHTML += '<div class="suggestion-section"><h4>Recent Searches</h4>';
            [...recentSearches].slice(-3).forEach(search => {
                suggestionsHTML += `
        ...
        if (data.artists?.items.length || data.tracks?.items.length) {
            suggestionsHTML += `
                <div class="suggestion-section">
                    <h4>Suggestions</h4>
                    ${data.artists.items.slice(0, 3).map(artist => `
                        <div class="suggestion-item" data-id="${artist.id}" data-type="artist">
                    ...
                    ${data.tracks.items.slice(0, 3).map(track => `
                        <div class="suggestion-item" data-id="${track.id}" data-type="track">
                            <img src="${track.album.images[0]?.url || 'default-track.png'}" alt="${track.name}">
...
async function searchSpotify(query = document.getElementById('searchInput').value, type = 'all') {
    if (!query) return;

    recentSearches.add(query);
    if (recentSearches.size > 10) {
        recentSearches.delete([...recentSearches][0]);
    }
    ...
    try {
        const types = type === 'all' ? 'artist,track,album' : type;
        const data = await fetchSpotifyData(`search?q=${encodeURIComponent(query)}&type=${types}&limit=20`);
...
function startVoiceSearch() {
    if ('webkitSpeechRecognition' in window) {
        ...
        recognition.onstart = () => {
            document.getElementById('voiceSearch').classList.add('listening');
        };
        ...
        recognition.onend = () => {
            document.getElementById('voiceSearch').classList.remove('listening');
        };
